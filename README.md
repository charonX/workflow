...
直接改代码、或直接命令 AI 改代码，都会被 AI 的误解吞掉--它当初就是误解了你的意图才写错的，你再用自然语言纠正，它可能继续误解。本工作流把人的纠错接口换成**测试**：人定规格锚点（什么算对），AI 从锚点机械推导断言（怎么测），AI 在契约内自主磨到全绿。
...
- **外层循环 -- 人控制的设计循环**：需求洞察 -> PRD -> UX -> 领域建模 -> 技术方案（仅 complex）->（自动链）REQ -> 断言签核 -> 验收 -> 回流。前半段（需求 / PRD / UX / 领域 / 技术）人做决定、改需求；REQ/test/断言签核由自动链完成，仅在升级点停下问人。
- **内层循环 -- agent 控制的实现循环**：读测试 -> 写代码 -> 跑测试 -> 改 bug -> 全绿。这一层 AI 自主迭代，人对代码只读。

> **人定规格锚点（PRD/design 的 expected 值）；AI 从锚点机械推导断言并在契约内实现。人不直接修改实现代码--他们修改需求和规格锚点，错误逐层回流到最高层；推导不出的点升级给人。**

五条承重不变量：

1. **规格锚点归人。** 人只持有「什么算对」这一行判断（PRD/design 的 expected 值锚点）；AI 从锚点机械推导断言并持有其余全部脚手架。AI 不得擅改规格锚点，不得从代码输出反推 expected。
...
- **真理只向下流（构建期）**：story 内 PRD -> REQ -> 测试 -> 代码，spec 是权威，代码不是真理来源。**story 完成后**：代码是逻辑真值（系统当前行为），意图真值归全局文档（adr/、business-capabilities.md、CONTEXT.md），已完成 story 的 spec 降级为历史记录（回流判断时查）。
- **纠错只向上回**：错误回到「出错的最高那一层」修，绝不在低层就地打补丁。

**快速收敛**（ADR 0005 / 0006 / 0007）：PRD 不可能一次全面，工作流不再前载完整。PRD 只需"可启动"——稳定块 + 主流程 + 复杂度分级即可结晶。识别出的缺口要么**就地补**（to-prd 对话 / `/review` 阶段），要么**显式归类**——就地补 / 移动块（§5）/ 新建 story / 范围外（§12），不许悬空。QA 验收发现的缺口走 `/bug` req-gap 就地补全，这是**默认收敛路径**，不是异常。门 1 签核默认由 AI 全量自检（expected 值 trace 到 PRD 锚点并交叉验证），仅在升级点（初衷漂移、契约歧义、expected 推导不出、安全边界、范围决策）停下问人。四道承重墙不可配置：测试前置、实现者对测试只读、PRD 对齐子代理、初衷锚点。
...
WAYFIND (可选) -> 外层循环：人控制的设计上下文
                    THINK -> PRD -> DESIGN -> DOMAIN-MODEL -> TECH-DESIGN(仅 complex) ->（自动链）CRYSTALLIZE -> TEST -> ASSERTION-SIGNOFF
                                                    │ 门 1：断言签核（AI 自检 + 升级点问人）
                                                    ▼
                                            内层循环：agent 控制的实现迭代
                                              BUILD -> QA
                                               ↑    │
...
| 6 | **CRYSTALLIZE** | `/crystallize` | 模型（自动链） | 把稳定 PRD 块转成带验收标准的 REQ-ID，expected 值源自 §6.3/§7/§10.4 锚点；每个 REQ 至少一个自动化测试；缺口就地补或升级 |
| 7 | **TEST** | `/test-author` | 模型（自动链） | 从 REQ + `prd.md` §11 seams 优先生成 CLI 测试骨架，expected 值 trace 到 PRD 锚点并标注 `EXPECTED-TRACE`；前端需求强制生成组件 / 浏览器结构行为测试；不能自动化的才允许在 REFLECT 中人工验收 |
| 8 | **ASSERTION-SIGNOFF** | `/signoff --stage=assertion` | 模型（自动链） | 门 1：AI 全量自检签核（含 expected 交叉验证）；升级点（初衷漂移 / 契约歧义 / expected 推导不出 / 安全边界 / 范围决策）停下问人；`phase` 置 BUILD |
...
| - | **末端统一审查** | `/review` | 用户（手动） | QA 全绿后、REFLECT 前，一次并行审全链（PRD/技术方案/REQ/测试/代码），可选但建议；`--cover=...` 可聚焦某层 |
...
| 角色 | 能写 | 不能写 | 职责 |
|---|---|---|---|
| **人** | 规格锚点（PRD/design 的 expected 值）、HTML UX | 实现代码 | 定锚点、验观感、拍板升级点、裁决 bug / 需求变更 |
| **测试作者 agent** | 测试脚手架（expected 值 trace 到锚点） | 实现代码 | 从规格锚点机械推导断言并展开成可执行测试 |
...
1. **门 1 - `/signoff --stage=assertion`**：断言签核默认 AI 全量自检（expected 值交叉验证 trace 到 PRD 锚点）；升级点（初衷漂移、契约歧义、expected 推导不出、安全边界、范围决策）停下问人。签核前不准实现。
2. **门 2 - `/reflect`**：QA 全绿、bug 处理结束后，人做最终验收确认并沉淀经验。不接受不合并。

人在边界使劲（门 1 只在升级点裁决、门 2 最终验收），中段全自主；失败走逃生口（实现者轮数上限->上报，不许自己改测试）。
...
| `/review` | REVIEW | 外层循环手动检查点；无 stage，按 cover 层并行派发 specialist 子代理审查（PRD/技术方案/REQ/测试/代码），汇总 `review.md` |
...
| `/signoff` | 签核 | 两个循环的切换点；门 1 断言签核（默认 AI 自检 + 升级点问人）把契约交给 AI，门 2 把 AI 产出交回人验收 |
...
| Skill | 阶段 | 用途 |
|---|---|---|
| `/crystallize` | 结晶 | 把 PRD 转成 REQ-ID，expected 值源自 §6.3/§7/§10.4 锚点；每个 REQ 至少一个自动化测试 |
| `/test-author` | TEST | 优先生成 CLI 测试骨架，expected 值 trace 到 PRD 锚点并标注 `EXPECTED-TRACE`；前端需求强制生成组件 / 浏览器结构行为测试；浏览器 E2E 默认 Playwright；不能自动化的才允许在 REFLECT 中人工验收 |
