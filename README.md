...
**一句话哲学**：还在开发中的 story，PRD 和测试是权威，AI 对着它们实现；story 完成后，代码就是当前行为的真值，当初的"为什么"记在全局文档里（ADR、能力地图、术语表），旧的 PRD 变成历史记录，供以后回溯。
...
- `stories/<id>/` 是单个 story 的产物。story 完成后标记 `completed`，它的 PRD 变成历史记录，以后逻辑看代码、意图看 `global/`。
