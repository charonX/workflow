...
PRD 已生成，需要把抽象需求转成可视化的 HTML 原型时。如果 PRD 里某个功能没有 UI（纯后台逻辑），跳过本 skill，直接 `/crystallize`。
...
1. **检查设计系统**：
   - 查找 `.aiassist/global/_ds_manifest.json`。缺失则调用 `/design-system` 先完成编译。
...
4. **检查模块/服务边界**：
   ...
   - 把发现的边界问题反馈回 PRD 第 6.1 节，必要时回流 `/to-prd`。
5. **明确方向和变体**：
   - 确认范围：哪些屏幕/流程需要设计。
...
- **设计系统 token 是绑定的**——不发明系统外颜色或样式；如需新 token，回 `/design-system`。
