...
单元测试和 E2E 测试验证功能正确性，但无法完全覆盖真实浏览器中的渲染、样式、console 错误、性能等运行时行为。`/browser-verify` 用 Chrome DevTools MCP 给 agent 一双"眼睛"，在 QA 阶段收集客观浏览器证据，供 bug 循环和 `/reflect` 最终验收参考。
...
- 把通用浏览器验证 skill 收窄为 loop-workflow 内 QA → bug 循环 → `/reflect` 之间的运行时验证门。
- 输出固定为 `.aiassist/stories/<id>/browser-verify-report.md`。
- 明确与 `.aiassist/stories/<id>/ux/*.html` HTML UX 原型对比；偏差通过 `/file-bug` 登记。
