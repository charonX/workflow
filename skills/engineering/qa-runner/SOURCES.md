...
内层实现循环的终点验证：行为机器判，偏差进 bug 循环。QA 不是额外步骤，而是契约的第二轮验证：跑 E2E、收集回归证据、处理不稳定测试，让"完成"有客观依据，为 bug 循环和 `/reflect` 最终验收提供事实基础。
...
- **v0.12.0 起**：QA 失败时建议调用 `/file-bug` 登记 bug；QA 全绿且无 open bug 后进入 `/reflect`。
