---
name: assertion-signoff
description: 断言签核门：确保所有断言都已被人签字，且 expected 值来源清晰，然后才允许进入 BUILD。
...
`/test-author` 完成，用户说"签断言"、`/assertion-signoff` 时。
...
4. **审查 expected 来源**：向用户确认每个关键 expected 值是人算/真实 JSON/已签标准，而非代码输出。
5. **展示检查清单**：让用户逐项确认。
...
- **不通过则禁止 BUILD**。
- 如果发现断言有问题，回退到 `/crystallize` 或 `/test-author`；绝不直接让实现者改测试。
- 用户签字 = 人对"什么算对"承担最终责任。

## 与参考项目的差异

- gstack `plan-eng-review` 是工程审查门；我们把它收窄为"断言签核门"（`/assertion-signoff`）。
