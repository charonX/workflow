...
2. **分析原型文件**：
   ...
   - 提取交互行为：点击、hover、动画、表单验证、状态转换
   - 提取数据流：state 变量、props 传递、事件处理
...
## Fidelity
- **高保真 (Hi-fi)**：像素级精确的 mockup，含最终颜色、字体、间距和交互。
  开发者应在目标环境中**像素级还原**。
- **低保真 (Lo-fi)**：线框图/布局骨架，展示结构和流程。
...
- 点击处理和导航流程
- 动画和过渡（duration、easing、properties）
- Hover 状态
- Loading 状态
- Error 状态
- 表单验证规则
...
|-------|------|------|--------|-------------|
| Heading | SF Pro Display | 32px | 700 | 1.25 |
| Body | SF Pro Text | 16px | 400 | 1.5 |
| ... | ... | ... | ... | ... |
...
| Hero Image | assets/hero.png | 用户提供 |
| ... | ... | ... |
...
- 状态覆盖完整——loading、empty、error、edge cases。
- 不添加设计中不存在的内容。
