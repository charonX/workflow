...
1. **用户主动建立/更新设计系统**
   - 用户说"建立设计系统"、"更新品牌系统"、"/design --mode=system"。
   - 进入**模式 A：建立/更新项目级设计系统**。

2. **story 进入 DESIGN 阶段**
   - 用户说"/design"，或 `/story` 路由到 DESIGN。
...
如果 PRD 里某功能完全没有 UI（纯后台逻辑），跳过本 skill，直接 `/crystallize`。
...
**如果用户没有提供任何来源且回答模糊，不要继续生成。** 停下来要求补充，或建议先 `/research` 做竞品/参考研究。
...
UI 流程常常暴露模块耦合问题。把发现的边界问题反馈回 PRD 第 6.1 节，必要时回流 `/to-prd`。
