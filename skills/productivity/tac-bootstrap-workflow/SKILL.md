---
name: tac-bootstrap-workflow
description: 在目标项目中初始化 test-as-contract workflow 的项目级基础设施，不创建具体 story。
...
- `templates/story/prd.md.template` → 保留在 workflow 内，由 `/tac-story` 使用
- `templates/story/requirements.md.template` → 保留在 workflow 内，由 `/tac-story` 使用
- `templates/story/workflow-state.yaml.template` → 保留在 workflow 内，由 `/tac-story` 使用
...
3. **复制对应模板**：
   ...
   - Node.js → `templates/github/workflows/tac-contract-gate.node.yml` → `.github/workflows/tac-contract-gate.yml`
   - Python → `templates/github/workflows/tac-contract-gate.python.yml` → `.github/workflows/tac-contract-gate.yml`
   - Swift → `templates/github/workflows/tac-contract-gate.swift.yml` → `.github/workflows/tac-contract-gate.yml`
   - Go → `templates/github/workflows/tac-contract-gate.go.yml` → `.github/workflows/tac-contract-gate.yml`
   - Generic → `templates/github/workflows/tac-contract-gate.generic.yml` → `.github/workflows/tac-contract-gate.yml`
...
- `.aiassist/global/*`
- `.aiassist/hooks/*`
- `.github/workflows/tac-contract-gate.yml`
- `CLAUDE.md`
...
| 标签 | 用途 | 可修改的文件 |
|---|---|---|
| `[tac-test]` | test-author 编写/修改测试、人签核断言 | 测试文件（`test/`、`*.test.*`、`e2e/` 等） |
| `[tac-build]` | implementer 编写/修改实现 | 实现代码（`src/`、`app/`、`lib/` 等） |
| `[tac-bootstrap]` | 工作流基础设施变更 | `.aiassist/`、`CLAUDE.md`、hooks 等 |
| `[tac-docs]` | PRD、需求、设计文档更新 | `.aiassist/stories/*/prd.md`、`.aiassist/stories/*/requirements.md` 等 |
| `[tac-ux]` | UX 原型更新 | `.aiassist/stories/*/ux/*.html`、`.aiassist/global/DESIGN.md` 等 |
...
3. `CLAUDE.md` 已更新
4. 下一步可以运行 `/tac-story` 开始第一个 story
