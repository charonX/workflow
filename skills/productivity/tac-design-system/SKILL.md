...
- `/tac-ux-explore` 开始 DESIGN 阶段时发现 `.aiassist/global/_ds_manifest.json` 缺失。
- 用户主动说"建立设计系统"、"更新设计系统"、"/tac-design-system"时。
...
9. **提示下一步**：设计系统已编译并预览就绪。用户可打开 `preview.html`；后续用 `/tac-ux-explore` 生成 story 级原型。
...
- `/tac-design-system` 是项目级、可反复更新的基础设施。
- `/tac-ux-explore` 读取 `.aiassist/global/_ds_manifest.json` 与 `_ds/<slug>/_ds_prompt.md`，将组件/token 用于 story 原型。
- story 原型通过 `ux/_ds/<slug>/` 的运行时拷贝引用设计系统，不直接依赖全局源路径。
- 如需新增全局 token 或组件，先回 `/tac-design-system`，再重新 `/tac-ux-explore`。

## 纪律

- 设计系统是**项目级资产**，不是某个 story 的产物。
- 不要在一个 story 里推翻整个设计系统；如需调整，应回到 `/tac-design-system`。
...
- 没有 `_ds_manifest.json` 的设计系统不能进入 `/tac-ux-explore` 的 BUILD 阶段。
