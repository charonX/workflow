---
name: tac-research
description: 针对一个技术/API/库/领域问题，启动 background agent 读取 primary sources，输出带引用的调研笔记到 story 的 research/ 目录。在 /tac-tech-design 前使用，为技术方案提供事实基础。
...
- 准备进入 `/tac-tech-design`，但对某个技术点/第三方 API/库还不清楚。
...
> 6. 不要下工程决策，只陈述事实。工程决策由人在 `/tac-tech-design` 中做。
...
- 仍存在的开放问题或不确定处
- 建议下一步：`/tac-tech-design` 或继续补充调研

### 5. 不自动消费

**`/tac-research` 只产出笔记，不修改 PRD/tech-design。** 后续由 `/tac-to-prd` 或 `/tac-tech-design` 显式读取该笔记并融入决策。
...
4. **不替代访谈**：用户痛点相关的问题回 `/tac-demand-insight`。
5. **输出是临时探索产物**：属于一挡，可被后续 `/tac-tech-design` 推翻或细化。

## 与相邻 skill 的边界

| Skill | 负责 | 不负责 |
|---|---|---|
| `/tac-demand-insight` | 用户痛点、需求边界、隐性假设 | 技术事实 |
| `/tac-research` | 技术/API/库/领域的事实调研 | 工程决策、用户访谈 |
| `/tac-tech-design` | 基于事实做技术方案、模块边界、接口契约 | 读文档找事实 |
| `/tac-to-prd` | 把调研笔记和访谈整合成 PRD | 不做原始调研 |

## 示例

```bash
/tac-research --topic="Claude Code Marketplace plugin.json schema" --story=2026-07-05-marketplace --sources="https://anthropic.com/claude-code/marketplace.schema.json"
```
