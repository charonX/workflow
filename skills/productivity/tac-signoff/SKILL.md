...
- `/tac-test-author` 完成后，进入 BUILD 前：`/tac-signoff --stage=assertion`
- QA 完成后，合并前：`/tac-signoff --stage=feel`
...
- assertion 阶段产生 `[tac-test] assertion-signoff for <story-id>` commit
...
- [ ] 无快照当判定依据。
- [ ] 边界/错误 case 已覆盖。
- [ ] `signoff.md` Assertion 部分已创建并通过 `[tac-test] assertion-signoff for <story-id>` commit 提交。
