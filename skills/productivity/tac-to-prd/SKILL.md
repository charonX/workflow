...
用户说"生成 PRD"、"把讨论整理成 PRD"、"/tac-to-prd"时。通常是在 `/tac-demand-insight` 之后。
...
4. **填充 PRD 模板**：使用 `templates/tac-story/prd.md.template`。
...
- **不访谈**：只合成已有信息。如果信息不足，回退到 `/tac-demand-insight`。
