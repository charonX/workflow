...
5. **识别模块/服务边界**：明确每个稳定块会触碰哪些 module/service/executive，是否会引入新的跨模块耦合。提前解耦的决策写进 PRD 第 10.1 节。
6. **复杂度分级**：根据模块数、输入量、分支数、外部依赖数，将 story 标记为 `simple` 或 `complex`，写入 PRD 第 9 节。
7. **填充 PRD 模板**：使用 `templates/story/prd.md.template`，必须填写第 6-9 节（操作流、验证规则、错误状态、复杂度分级）。
8. **PRD 完整性自检查**：对照“PRD 完整性检查清单”逐项确认；有 GAP 时必须在“移动块”或“补充说明”中给出 N/A 理由。
...
任何未通过项必须：
- 归入第 5 节“移动块”（暂不入 REQ），或
- 在第 13 节“补充说明”中给出 N/A 理由。
...
- **PRD 完整性红线**：`complex` story 必须完整填写第 6-8 节；`simple` story 至少填写第 8 节，其他可简写为 N/A 但需理由。未达红线，`/crystallize` 会拦截。
